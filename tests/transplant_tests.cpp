...
#define ASSERT_GLOBAL_OPTIONS(exp_global_opt) do { \
    cr_assert_eq(g_global_options, exp_global_opt, \
		 "Wrong value for global_options: expected 0x%x, was 0x%x\n", \
		 exp_global_opt, g_global_options); \
  } while(0)

#define ARGC(argv) (sizeof(argv)/sizeof((argv)[0]) - 1)

#define INIT_PATH_BUF(path) do { \
    std::strcpy(g_path_buf, (path)); \
    g_path_length = std::strlen(g_path_buf); \
  } while(0)
...
#define DESERIALIZE_RESULT(ref_prog) do { \
    std::FILE *f; size_t s = 0; char *cmd = nullptr; \
    NEWSTREAM(f, s, cmd); \
    std::fprintf(f, "%s -d -p %s < %s", ref_prog, alt_outfile, test_outfile); \
    std::fclose(f); \
...
{
    char *argv[] = {QUOTE(PROGRAM_NAME), "-h", "-s", "-c", nullptr};
    int argc = ARGC(argv);
...
Test(TEST_SUITE, TEST_NAME, .timeout=TEST_TIMEOUT)
{
    char *argv[] = {QUOTE(PROGRAM_NAME), "-s", nullptr};
    int argc = ARGC(argv);
    int ret = validargs(argc, argv);
    ASSERT_RETURN(0);
...
Test(TEST_SUITE, TEST_NAME, .timeout=TEST_TIMEOUT)
{
    char *argv[] = {QUOTE(PROGRAM_NAME), "-s", "-p", "./testdir/", nullptr};
    int argc = ARGC(argv);
    int ret = validargs(argc, argv);
    ASSERT_RETURN(0);
    ASSERT_GLOBAL_OPTIONS(SERIALIZE_MODE);
...
Test(TEST_SUITE, TEST_NAME, .timeout=TEST_TIMEOUT)
{
    char *argv[] = {QUOTE(PROGRAM_NAME), "-d", nullptr};
    int argc = ARGC(argv);
    int ret = validargs(argc, argv);
    ASSERT_RETURN(0);
...
Test(TEST_SUITE, TEST_NAME, .timeout=TEST_TIMEOUT)
{
    char *argv[] = {QUOTE(PROGRAM_NAME), "-d", "-c", "-p", "./out/", nullptr};
    int argc = ARGC(argv);
    int ret = validargs(argc, argv);
    ASSERT_RETURN(0);
...
Test(TEST_SUITE, TEST_NAME, .timeout=TEST_TIMEOUT)
{
    char *argv[] = {QUOTE(PROGRAM_NAME), "-s", "-c", nullptr};
    int argc = ARGC(argv);
    int ret = validargs(argc, argv);
    ASSERT_RETURN(-1);
...
Test(TEST_SUITE, TEST_NAME, .timeout=TEST_TIMEOUT)
{
    char *argv[] = {QUOTE(PROGRAM_NAME), "-d", "-h", nullptr};
    int argc = ARGC(argv);
    int ret = validargs(argc, argv);
    ASSERT_RETURN(-1);
    //ASSERT_GLOBAL_OPTIONS(0x0);
...
Test(TEST_SUITE, TEST_NAME, .timeout=TEST_TIMEOUT)
{
    char *argv[] = {QUOTE(PROGRAM_NAME), "-d", "-p", nullptr};
    int argc = ARGC(argv);
    int ret = validargs(argc, argv);
    ASSERT_RETURN(-1);
    //ASSERT_GLOBAL_OPTIONS(0x0);
...
Test(TEST_SUITE, TEST_NAME, .timeout=TEST_TIMEOUT)
{
    char *argv[] = {QUOTE(PROGRAM_NAME), "-s", "-p", nullptr};
    int argc = ARGC(argv);
    int ret = validargs(argc, argv);
    ASSERT_RETURN(-1);
    //ASSERT_GLOBAL_OPTIONS(0x0);
...
Test(TEST_SUITE, TEST_NAME, .timeout=TEST_TIMEOUT)
{
    char *argv[] = {QUOTE(PROGRAM_NAME), "-s", "-p", "-c", nullptr};
    int argc = ARGC(argv);
    int ret = validargs(argc, argv);
    ASSERT_RETURN(0);
    ASSERT_GLOBAL_OPTIONS(SERIALIZE_MODE);
...
Test(TEST_SUITE, TEST_NAME, .timeout=TEST_TIMEOUT)
{
    char *argv[] = {QUOTE(PROGRAM_NAME), "-d", "-c", "-p", nullptr};
    int argc = ARGC(argv);
    int ret = validargs(argc, argv);
    ASSERT_RETURN(-1);
    //ASSERT_GLOBAL_OPTIONS(0x0);
...
Test(TEST_SUITE, TEST_NAME, .timeout=TEST_TIMEOUT)
{
    ...
    fflush(stdout);
    ASSERT_RETURN(0);
    assert_files_match(ref_outfile, test_outfile, nullptr);
}
#undef TEST_NAME
...
Test(TEST_SUITE, TEST_NAME, .timeout=TEST_TIMEOUT)
{
    ...
    assert_expected_status(EXIT_SUCCESS, status);
    assert_files_match(ref_errfile, test_errfile, nullptr);
}
#undef TEST_NAME
...
Test(TEST_SUITE, TEST_NAME, .timeout=TEST_TIMEOUT)
{
    ...
    char *cf = nullptr; s = 0; NEWSTREAM(f, s, cf);
